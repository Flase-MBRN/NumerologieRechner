...
function initForm() {
  ...
  form.addEventListener('submit', async e => {
    ...
    appendExtraTile('Hidden Passion',   calculateHiddenPassion(name), 'Häufigste Zahl(en) im Namen',       'hiddenPassion',   'hiddenPassion',   false, 'acc-talents-grid');
    appendExtraTile('Cornerstone',      String(calculateCornerstone(name)), 'Erster Buchstabe Vorname',    'cornerstone',     'cornerstone',     false, 'acc-talents-grid');
    appendExtraTile('Capstone',         String(calculateCapstone(name)),    'Letzter Buchstabe Nachname',  'capstone',        'capstone',        false, 'acc-talents-grid');
    appendExtraTile('First Impression', formatValue(calculateFirstImpression(name)), 'Erste Buchstaben Vor- & Nachname', 'firstImpression', 'firstImpression', false, 'acc-talents-grid');
    ...
    appendExtraTile('Life Path Bridge',        String(lpBridge), '|Lebenszahl − Ausdruckszahl|',       'bridge', 'bridge', false, 'acc-talents-grid');
    appendExtraTile('Soul-Personality Bridge', String(spBridge), '|Seelenzahl − Persönlichkeitszahl|', 'bridge', 'bridge', false, 'acc-talents-grid');
    ...
    appendExtraTile('Life Cycle 1', formatValue(lc.lc1), 'Kindheit & Jugend',    'lifeCycle', 'lifeCycle', false, 'acc-lifecycle-grid');
    appendExtraTile('Life Cycle 2', formatValue(lc.lc2), 'Erwachsenenphase',     'lifeCycle', 'lifeCycle', false, 'acc-lifecycle-grid');
    appendExtraTile('Life Cycle 3', formatValue(lc.lc3), 'Spätes Leben',         'lifeCycle', 'lifeCycle', false, 'acc-lifecycle-grid');
    ...
    appendExtraTile('Rational Thought', formatValue(calculateRationalThought(date, expressionSum)), 'Geburtstagszahl + Ausdruckszahl', 'rationalThought','rationalThought', false, 'acc-soul-grid');
...
function drawShareCard(name, lifeVal, archTitle, teaser, soulVal, exprVal) {
  ...
  ctx.fillStyle = '#e8e8f4'; ctx.font = '600 62px sans-serif';
  ctx.fillText(archTitle || '', W/2, H*0.638);
  ctx.shadowBlur = 0;

  /* Teaser */
  ctx.fillStyle = 'rgba(168,180,196,0.75)'; ctx.font = '300 34px sans-serif';
  wrapCanvasText(ctx, teaser || '', W/2, H*0.700, 820, 52);

  /* Secondary numbers */
  if (soulVal || exprVal) {
    ctx.fillStyle = 'rgba(96,165,250,0.55)'; ctx.font = '600 30px sans-serif';
    const parts = [];
  ...
  /* Name */
  ctx.fillStyle = 'rgba(167,139,250,0.60)'; ctx.font = '500 38px sans-serif';
  ...
  /* Footer */
  ctx.fillStyle = 'rgba(245,158,11,0.38)'; ctx.font = '600 28px sans-serif';
